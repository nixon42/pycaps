...
class LimitByCharsSplitterConfig(BaseConfigModel):
    ...
    @field_validator(
        "min_chars", "max_chars", "avoid_finishing_segment_with_word_shorter_than"
    )
    @classmethod
...
SplitterConfig = Annotated[
    LimitByCharsSplitterConfig
    | LimitByWordsSplitterConfig
    | SplitIntoSentencesSplitterConfig,
...
SoundEffectConfig = Annotated[
    PresetSoundEffectConfig | CustomSoundEffectConfig, Field(discriminator="type")
]
...
class BaseAnimationPrimitiveConfig(BaseConfigModel):
    ...
    duration: float = 0.2
    delay: float = 0.0
    transformer: Literal["linear", "ease_in", "ease_out", "ease_in_out", "inverse"] = (
        "linear"
    )
    what: ElementType
    when: EventType
...
TaggerRule = Annotated[
    AiTaggerRuleConfig | RegexTaggerRuleConfig | WordlistTaggerRuleConfig,
    Field(discriminator="type"),
]
