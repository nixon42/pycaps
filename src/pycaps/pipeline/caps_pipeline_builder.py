...
from pycaps.renderer import SubtitleRenderer, PictexSubtitleRenderer
...
class CapsPipelineBuilder:
    ...
    def with_output_video(
        self, output_video_path: str, overwrite: bool = False
    ) -> "CapsPipelineBuilder":
        if os.path.exists(output_video_path):
            if overwrite:
                ...
                raise ValueError(
                    f"Output video path already exists: {output_video_path}"
                )
    ...
    def with_layout_options(
        self, layout_options: SubtitleLayoutOptions
    ) -> "CapsPipelineBuilder":
    ...
    def with_custom_subtitle_renderer(
        self, subtitle_renderer: SubtitleRenderer
    ) -> "CapsPipelineBuilder":
        self._caps_pipeline._renderer = subtitle_renderer
        return self

    def with_pictex_renderer(self) -> "CapsPipelineBuilder":
        self._caps_pipeline._renderer = PictexSubtitleRenderer()
        return self

    def with_whisper_config(
        self,
    ...
    def with_custom_audio_transcriber(
    ...
    def with_transcription(
        self,
        transcription: Document | dict | str,
        format: TranscriptFormat | str = TranscriptFormat.AUTO,
    ) -> "CapsPipelineBuilder":
        self._caps_pipeline._transcription_for_loading = load_transcription(
            transcription, format
        )
        return self

    def with_transcription_file(
        self, path: str, format: TranscriptFormat | str = TranscriptFormat.AUTO
    ) -> "CapsPipelineBuilder":
    ...
    def add_animation(
        ...
        self._caps_pipeline._animators.append(
            ElementAnimator(animation, when, what, tag_condition)
        )
        return self
    ...
    def build(self, preview_time: Optional[tuple[float, float]] = None) -> CapsPipeline:
        ...
        if preview_time:
            logger().warning(
                "Generating preview: using dummy text and reducing quality to save time."
            )
